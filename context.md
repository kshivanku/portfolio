...
- The page starts with a client-side password overlay; the portfolio content, theme toggle, menu, and project indicator stay hidden until the correct password is entered
...
- Top-left menu icon is hidden on the hero, visible from projects through the resume/contact area, opens a full-screen menu overlay with Work, Play, Resume, and Contact links, and morphs into an X while open; Play opens `https://kshivanku.github.io/begrateful/` in a new tab; the overlay fades smoothly and each link uses a GSAP masked reveal with slight stagger; tapping it or pressing Escape closes the overlay
...
*Last updated: Added subtle 3D z-axis movement and rotation to hero letter drift.*
